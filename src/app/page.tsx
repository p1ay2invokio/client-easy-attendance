    ...
    <div>
      ...
      <div className="flex flex-col gap-0 justify-center items-center h-[calc(100vh-300px)]">
        ...
        <div className="w-[360px] p-2  rounded-xl border bg-white shadow-sm border-gray-200 flex flex-col justify-start items-center gap-5">
            ...
            <table className="w-full text-center ">
              ...
              <tbody>
                <tr className="bg-white font-[light] text-[18px] text-gray-600">
                  {attendance ? attendance.in_timestamp || attendance.out_timestamp ?
                    <>
                    ...
                    </> : <td colSpan={2} className="h-[50px]">ไม่พบบันทึกเวลางานในวันนี้</td> : null}
          ...
          {attendance ? !attendance.in_timestamp ? <button className="w-[90%] h-[48px] bg-blue-400/20 border-blue-300 text-blue-500 border rounded-lg font-[regular] text-[20px] cursor-pointer hover:bg-blue-400/30 duration-100" onClick={async () => {
            if (myLat && myLong) {
              let response: any = await new AttendanceMethod().attend(attendance.id, myLat, myLong)
          ...
          }}>ออกงาน</button> : <p className="font-[light] text-green-600 text-[20px]">คุณได้ลงเวลาเข้าออกงานเรียบร้อยแล้ว !</p> : <p className="font-[light] text-sky-600 text-[20px]">โปรดรอวันถัดไป</p>}

        </div>
      </div>

      {/* {attendance ? attendance.employee.role == 1 ?       <div onClick={()=>{
        navigate.push("/admin")
      }} className="fixed right-10 bottom-30">
        <MdDashboard size={40}/>
      </div> : null : null} */}
